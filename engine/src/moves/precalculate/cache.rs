...
use super::{
    bishop::create_bishop_potential_blockers_mask,
    rook::create_rook_potential_blockers_mask,
};
...
impl PrecalculatedCache {
    pub fn create() -> PrecalculatedCache {
        ...
        let seed: u64 = SystemTime::now().duration_since(SystemTime::UNIX_EPOCH).unwrap().as_secs();
        ...
        let (bishop_magics, bishop_magic_attack_tables) =
            find_bishop_magic_numbers(&mut rng, &bishop_bit_counts, &bishop_potential_blockers_masks);
